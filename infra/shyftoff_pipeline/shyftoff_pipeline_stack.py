...
class ShyftoffPipelineStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs):
        ...
        generate_query_task = tasks.LambdaInvoke(
            ...
            output_path="$.Payload"  # Only pass Lambda's payload forward
        )
        ...
        # Pass state to forward 'athena_query' from Lambda to next step
        forward_query = sfn.Pass(
            self, "ForwardQuery",
            parameters={
        ...
        # Athena task: run CTAS query generated by Lambda
        athena_ctas_task = tasks.CallAwsService(
            self, "RunAthenaCTAS",
            service="athena",
            action="startQueryExecution",
            parameters={
                "QueryString.$": "$.athena_query",  # Read from Pass state
                "ResultConfiguration": {
                    "OutputLocation": f"s3://{gold_bucket.bucket_name}/athena/"
                }
            },
        ...
        # Step Function definition: Lambda -> MSCK -> Pass -> CTAS
        step_fn_definition = generate_query_task.next(msck_repair_task).next(forward_query).next(athena_ctas_task)
        ...
        # Update Glue-to-StepFn Lambda with the Step Function ARN
