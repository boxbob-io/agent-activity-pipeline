...
class ShyftoffPipelineStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs):
        ...
        bronze_bucket = s3.Bucket.from_bucket_name(
            self, "BronzeBucket",
            bucket_name=os.environ.get("BRONZE_BUCKET", "shyftoff-pipeline-bronze-dev")
        )
        silver_bucket = s3.Bucket.from_bucket_name(
            self, "SilverBucket",
            bucket_name=os.environ.get("SILVER_BUCKET", "shyftoff-pipeline-silver-dev")
        )
        scripts_bucket = s3.Bucket.from_bucket_name(
            self, "ScriptsBucket",
            bucket_name=os.environ.get("SCRIPTS_BUCKET", "shyftoff-pipeline-scripts-dev")
        )
        ...
        # -----------------------------
        # EventBridge rule for CSV upload
        # -----------------------------
        rule = events.Rule(
